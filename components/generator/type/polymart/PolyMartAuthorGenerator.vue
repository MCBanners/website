<template>
  <div>
    <form-wizard
      ...
      subtitle="Create a Polymart Author Banner"
      shape="tab"
      color="#4299e1"
      error-color="#ec4e20"
...
export default {
  ...
  computed: {
    ...
    baseURL() {
      return this.generateBannerUrl('author-polymart', {
        id: this.author.id,
      })
    },
  },
  methods: {
    ...
    async checkValidAuthor() {
      ...
      if (!id) {
        this.author.error = 'Please enter a Polymart Author ID.'
        return false
      }

      const valid = await this.$store.dispatch('checkValidPolyMartAuthor', id)

      if (valid.state) {
        return true
      } else {
        this.author.error =
          "That doesn't seem to a valid Polymart Author ID. Please double check it."
        return false
      }
    },
    async handleComplete() {
      this.loading = true
      await this.savePolyMartAuthorBanner()
      this.loading = false
