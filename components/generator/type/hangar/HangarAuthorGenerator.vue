<template>
  <div>
    <form-wizard
      ...
      subtitle="Create a Hangar Author Banner"
      shape="tab"
      color="#4299e1"
      error-color="#ec4e20"
    ...
    >
      <tab-content :before-change="checkValidAuthor" title="Author Details">
        ...
        >
          <AuthorGeneratorStepOne type="hangar" @update="updateAuthorDetails" />
        </GeneratorPreCheck>
      </tab-content>
...
export default {
  ...
  computed: {
    ...
    baseURL() {
      return this.generateBannerUrl('author-hangar', {
  ...
  methods: {
    ...
    async checkValidAuthor() {
      ...
      if (!username) {
        this.author.error = 'Please enter a Hangar Author Name.'
        return false
      }

      const valid = await this.$store.dispatch(
        'checkValidHangarAuthor',
        username
      )
      ...
      if (valid.state) {
        ...
        this.author.error =
          "That doesn't seem to a valid Hangar Author Name. Please double check it. T"
        return false
      }
    },
    async handleComplete() {
      this.loading = true
      await this.saveHangarAuthorBanner()
      this.loading = false
