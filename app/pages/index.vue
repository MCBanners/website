...
const { validateResource, validateAuthor, validateServer } =
  useBannerEntryFlow()
...
const selectedType = computed(
  () => bannerTypes.find((type) => type.kind === kind.value)!,
)
...
const activePlatform = computed({
  get: () =>
    kind.value === 'author' ? authorPlatform.value : resourcePlatform.value,
  set: (value: string) => {
    if (kind.value === 'author') {
...
const resourcePlaceholder = computed<SourcePlaceholder>(
  () =>
    resourcePlaceholders[resourcePlatform.value] || fallbackResourcePlaceholder,
)
const authorPlaceholder = computed<SourcePlaceholder>(
  () => authorPlaceholders[authorPlatform.value] || fallbackAuthorPlaceholder,
)

function platformLabel(value: string): string {
  return (
    activePlatforms.value.find(
      (platformOption) => platformOption.value === value,
    )?.label || value
  )
}
...
async function continueToBuilder() {
  ...
  if (kind.value === 'resource') {
    const bannerId =
      ...
      extractResourceIdFromUrl(resourceUrl.value, resourcePlatform.value)
    ...
    await validateAndOpenBuilder(() =>
      validateResource(resourcePlatform.value, bannerId),
    )
    return
  }
  ...
  if (kind.value === 'author') {
    ...
    await validateAndOpenBuilder(() =>
      validateAuthor(authorPlatform.value, bannerId),
    )
    return
  }
...
async function validateAndOpenBuilder(
  ...
  try {
    ...
    validationMessage.value =
      result.message ||
      'Could not validate that source. Check the details and try again.'
    flowState.value = 'error'
  } catch {
    validationMessage.value =
      'Could not reach the banner API. Check the source details and try again.'
    flowState.value = 'error'
  }
}
</script>
...
<template>
  <main
    class="mcb-home-surface bg-gray-950 text-white"
  ...
  >
    ...
    >
      <div
        class="grid gap-5 lg:grid-cols-[minmax(0,0.92fr)_minmax(520px,1fr)] lg:items-end"
      >
        <div class="space-y-4">
          <div class="space-y-3">
            <h1
              class="max-w-[820px] text-4xl font-bold leading-tight tracking-normal text-white"
            >
              Build beautiful
              <span class="text-lime-300">live-updating</span> banners in
              seconds
            </h1>
            <p class="max-w-2xl text-base leading-7 text-gray-300 sm:text-lg">
              Turn backend stats into polished banner images for forums,
              websites, and Minecraft communities.
            </p>
          </div>
        </div>
        ...
        <div class="grid gap-3 sm:grid-cols-3">
          ...
          >
            <div
              class="flex size-10 shrink-0 items-center justify-center rounded-lg border border-lime-400/25 bg-lime-400/10 text-lime-300"
            >
      ...
      >
          ...
          <ol class="grid gap-3 sm:grid-cols-3">
            ...
            >
              <span
                ...
                :class="
                  index === 0
                    ? 'border-lime-300 bg-lime-300 text-gray-950'
        ...
        >
          <div
            class="flex size-16 items-center justify-center rounded-full border border-lime-300/30 bg-lime-300/10 text-lime-300"
          >
          ...
          <div>
            ...
            <p class="mt-2 text-gray-400">Preparing your banner builder...</p>
          </div>
        </div>
        ...
        >
          <div class="space-y-7">
            <div>
              <p
                class="text-sm font-semibold uppercase tracking-wide text-lime-300"
              >
                Step 1
              </p>
              ...
              <p class="mt-2 max-w-3xl text-gray-400">
                Pick the banner type, select the supported source, and open the
            ...
            <div class="grid gap-3 lg:grid-cols-3">
              ...
              >
                <span
                  class="flex size-11 shrink-0 items-center justify-center rounded-lg border border-white/10 bg-white/5"
                >
                ...
                <span>
                  ...
                  <span class="mt-1 block text-sm leading-5 text-gray-400">{{
            ...
            <div v-if="kind !== 'server'" class="space-y-3">
              ...
              >
                <button
                  ...
                  :class="
                    activePlatform === platformOption.value
          ...
          <div class="rounded-lg border border-white/10 bg-gray-900/70 p-5">
            <div class="space-y-5">
              ...
              <div v-if="kind === 'resource'" class="space-y-4">
                ...
                <div
                  class="flex items-center gap-3 text-xs uppercase tracking-wide text-gray-500"
                >
              ...
              <p class="text-xs leading-5 text-gray-500">
                We will validate the
                {{ selectedType.label.toLowerCase() }} source and open the
      ...
      <div class="space-y-3">
        ...
        >
          <div>
            <p
              class="text-xs font-semibold uppercase tracking-wide text-lime-300"
            >
              Generated examples
            </p>
            <h2 class="text-lg font-bold text-white">Real MCBanners output</h2>
          </div>
        </div>
        <div class="grid gap-4 md:grid-cols-3">
          ...
          >
            ...
            <div
              class="aspect-[3/1] overflow-hidden rounded-md border border-white/10 bg-gray-950 p-2"
            >
              <img
                :src="card.imageUrl"
                :alt="card.alt"
