...
const items = computed(() => [
  {
    ...
    description: 'Enter the Author ID that you want to generate a banner for.'
  },
  {
    value: 'configure',
    label: 'Configure Banner',
    disabled: !canConfigure.value,
    description: 'Configure the banner to your liking. You can change the background, author logo, author name, resource count, review count, likes, stars, views, and download count of your banner.'
  }
])
...
async function validateDetails () {
  ...
  if (!bannerId) {
    ...
    toast.add({
      id: 'fetch_failed',
      title: 'Error!',
      description: 'Enter an author ID before fetching.',
      duration: 3000
    })
    return
  }
  ...
  if (json.valid) {
    ...
    toast.add({
      id: 'fetch_failed',
      title: 'Error!',
      description: 'Failed to fetch author. Check that the author ID is correct.',
      duration: 3000
    })
  }
}
...
<template>
  <div
    ...
    data-testid="author-builder-page"
  >
    <UTabs
      v-model="activeTab"
      :items="items"
          ...
          <UCard>
            ...
            <div class="space-y-4">
              <UAlert
                ...
                title="Heads Up"
                description="While we refer to it as an author ID, it can also be a slug or name based on the platform."
                color="primary"
                variant="outline"
              />
              ...
              <UFormField label="Author ID" name="id">
                <UInput
                  v-model="idForm.id"
                  name="id"
                  aria-label="Author ID"
                  class="w-full"
                  data-testid="author-id-input"
                />
              </UFormField>
            </div>
            ...
            <template #footer>
              <UButton
                type="button"
                variant="outline"
                data-testid="author-fetch-button"
          ...
          <UCard>
            ...
            <template #footer>
              <UButton
                type="button"
                variant="outline"
                data-testid="author-save-button"
                @click="saveCurrent"
              >
                Submit
              </UButton>
