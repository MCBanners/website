...
const embedCode = computed(() => {
  ...
  if (selectedFormat.value === 'html')
    return `<img src='${props.savedUrl}' alt='Banner' />`
...
async function copyPreviewUrl() {
  await copyText(
    defaults.generateBannerUrl(),
    'preview',
    'Preview URL copied to your clipboard.',
  )
}
...
async function copySavedUrl() {
  await copyText(
    props.savedUrl,
    'saved-url',
    'Saved banner URL copied to your clipboard.',
  )
}
...
async function copyEmbedCode() {
  await copyText(
    embedCode.value,
    `embed-${selectedFormat.value}`,
    `${formatItems.find((item) => item.value === selectedFormat.value)?.label} snippet copied to your clipboard.`,
  )
}
</script>
...
<template>
  ...
  >
    <template #header>
      <div>
        <h3 class="text-base font-semibold text-highlighted">Preview</h3>
      ...
      <template v-if="savedUrl">
        ...
        <div class="space-y-2">
          <div class="flex items-center gap-2">
            ...
            <UBadge color="neutral" variant="soft"> Permanent URL </UBadge>
          </div>
          <div class="flex gap-2">
            ...
            <UButton
              ...
              color="neutral"
              :icon="
                copiedKey === 'saved-url' ? 'i-lucide-check' : 'i-lucide-copy'
              "
              @click="copySavedUrl"
            >
              Copy
        ...
        <div class="space-y-3">
          ...
          >
            <p class="text-sm font-semibold text-highlighted">Embed Code</p>
          ...
          <div class="flex gap-2">
            ...
            <UButton
              ...
              color="neutral"
              :icon="
                copiedKey === `embed-${selectedFormat}`
                  ? 'i-lucide-check'
                  : 'i-lucide-copy'
              "
              @click="copyEmbedCode"
